...
def demo_load_audio_output():
	...
	"""
	audio_array = load_audio("res/audio/voice-message-01.mp3", sr=44100) #44.1kHz is a typical native sampling rate for audio, so I am using sr (samplingrate) = 44100 for demonstration purposes
	...
	audio_array = load_audio("res/audio/voice-message-01.mp3", sr=16000) # Whisper internally operates at 16kHz
	...
	audio_array = load_audio("res/audio/voice-message-01.mp3") # if no sampling rate is provided, Whisper will use its default sampling rate of 16000/16kHz.
...
def demo_pad_array():
	audio_array = load_audio("res/audio/voice-message-01.mp3") # if no sampling rate is provided, Whisper will load audio using its default sampling rate of 16000/16kHz.
	print(f"{len(audio_array)} amplitude samples when sampled at default Whisper sample rate") 
	#592248 samples
	...
	audio_array = load_audio("res/audio/voice-message-04.ogg") # if no sampling rate is provided, Whisper will use its default sampling rate of 16000/16kHz.
	print(f"{len(audio_array)} amplitude samples when sampled at default Whisper sample rate") 
	#158400 samples
...
def demo_compare_audio_data_creation_methods():
	...
	def demo_cast_to_audio():
		...
		dataset_mapped = dataset.map(partial(extract_features_and_tokenize, processor=processor), remove_columns=["audio","transcript","speech_ends_at","model_name","manually_verified"], num_proc=4)
